...
\bottomrule
\end{tabular}
\caption{Effect of the forgetting ratio $\omega$ on English (50k cap, 10k
document-epochs). As forgetting strengthens, dead weight (never-used supra-word
types) falls monotonically, and beyond a threshold the vocabulary stops filling
the cap and self-regulates to a far smaller emergent size. [EN]}
\label{tab:omega}
\end{table}
...
The results in this paper use the implementation in which active re-ranking and
passive probation are both operating (as described above); we release this
implementation with the paper. \TODO{once the multilingual re-run completes, replace all LiB columns
in Tables~\ref{tab:dl_multilingual}, \ref{tab:bpc_multi}, etc.\ with the
forgetting-active numbers and state the $\omega$/\texttt{life}/\texttt{doc\_size}
used per language.}
